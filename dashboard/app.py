    ...
    with col_left:
        ...
        for beta_name, coeff_value in scenario.visible_betas.items():
            if beta_name in beta_labels:
                ...
                coeff_rows.append(
                    f'<div class="tech-log-row"><span class="tech-log-key">{label}</span>'
                    f'<span class="tech-log-value">{current_val}</span>'
                    f'<span class="tech-log-coeff">{sign}{coeff_value:.1f}</span></div>'
                )
        coeff_html = "".join(coeff_rows)
        ...
        st.markdown(tech_log_html, unsafe_allow_html=True)
