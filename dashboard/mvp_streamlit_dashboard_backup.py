...
def make_facet_plot(facet_df: pd.DataFrame, title: str) -> Tuple[str, int]:
    ...
    min_date = facet_df["date"].min()
    max_date = facet_df["date"].max()
    weekly_breaks: List[datetime] = []
    if pd.notna(min_date) and pd.notna(max_date):
        start_sunday = min_date - pd.Timedelta(days=(min_date.weekday() + 1) % 7)
        weekly_range = pd.date_range(
            start_sunday, max_date + pd.Timedelta(days=7), freq="7D"
        )
        weekly_breaks = weekly_range.to_pydatetime().tolist()
    ...
    plot = (
        ...
        + geom_point(size=2.2, tooltips=tooltip, alpha=0.85, show_legend=False)
        + geom_text(
            data=weekday_df,
    ...
    if weekly_breaks:
        plot = plot + scale_x_datetime(breaks=weekly_breaks)
